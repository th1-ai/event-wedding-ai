---
name: event-wedding-ai
description: Run Event & Wedding AI ("The Event Planner") — Owns the whole event once someone wants to hold it here — weddings, conferences, offsites, board meetings.. Use when the user asks to run the agent, check what is waiting for review, approve or reject a draft, or asks how the agent is doing. Trigger phrases: "run The Event Planner", "/event-wedding-ai", "check the queue", "what is waiting for me", "approve that draft".
---

# Event & Wedding AI

Runs Event & Wedding AI and works its review queue. Everything happens from the repo
