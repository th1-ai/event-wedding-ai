...
| File | What it holds |
|---|---|
| `property.md` | The facts. Rooms, times, prices, policies, directions, what is nearby. Has an "Events and function spaces" section too. |
...
| `event-pricing-policy.md` | The reasoning behind the numbers in `config/agent.yaml: rates:`/`spaces:` - how quoting works, the negotiation band, what is never done. Loaded into the intake and desk-note prompt context. |
| `event-spaces.md` | A human-readable reference for your venue inventory, and the restaurant-lens variant. The agent itself reads `config/agent.yaml`, not this file - keep the two in sync by hand. |
